...
class VideoCompressor(nn.Module):
    ...
    def forward(self, input_image, referframe, quant_noise_feature=None, quant_noise_z=None, quant_noise_mv=None, get_IND=False):
        #used for training
        estmv = self.opticFlow(input_image, referframe) #motion vector, the channel is 2
        mvfeature = self.mvEncoder(estmv)
        if self.training or get_IND:
    ...
    #Below are used for inferencing
    def encode(self, input_image, refer_frame, return_z = False, mask = None, scale_factor=1, return_dec = False):
        ...
        #print(f'Test: estmv size  is {estmv.size()}')
        mvfeature = self.mvEncoder(estmv)
        #print(f'Test: mvfeature size  is {mvfeature.size()}')
        quant_mv = torch.round(mvfeature)
        ...
        input_residual = input_image - prediction
        #print(f'Test: input_image size  is {input_image.size()}')
        #print(f"Test: input_residual size  is {input_residual.size()}")
        if mask is not None:
            input_residual = input_residual * mask
            print("Here, mask = ", mask)

        feature = self.resEncoder(input_residual)
        #print(f"Test: Encode residual size  is {feature.size()}")
        feature_renorm = feature
        #compressed_feature_renorm = torch.round(feature_renorm)
