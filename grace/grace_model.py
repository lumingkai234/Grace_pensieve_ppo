...
class GraceEntropyCoder:
    ...
    def estimate_res(self, res, sigma):
        ...
        bpp_feature = torch.clamp(-1.0 * torch.log(probs + 1e-5) / math.log(2.0), 0, 50)

        total_bits = torch.sum(bpp_feature)
        total_bits = total_bits.item()
        return None, total_bits / 8, bpp_feature

    def estimate_mv(self, mv):
        prob = self.model.bitEstimator_mv(mv + 0.5) - self.model.bitEstimator_mv(mv - 0.5)

        bpp_feature = torch.clamp(-1.0 * torch.log(prob + 1e-5) / math.log(2.0), 0, 50)

        total_bits = torch.sum(bpp_feature)
        return None, total_bits.item() / 8, bpp_feature
    ...
    def entropy_encode(self, code, shape_mv, shape_res, z, use_estimation = False):
        ...
        if use_estimation:
            bs1, sz1, bpp_res = self.estimate_res(res, sigma)
            bs2, sz2, bpp_mv = self.estimate_mv(mv)
            bs3, sz3 = self.estimate_z(z)
        else:
            bs1, sz1 = self.compress_res(res, sigma)
            bs2, sz2 = self.compress_mv(mv)
            bs3, sz3 = self.compress_z(z)
        return None, sz1 + sz2 + sz3, bpp_mv, bpp_res
